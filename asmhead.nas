...
;AT互換機の仕様でPICを初期化する時,これをCLI前にやらないと偶にハングアップする
;PICの初期化はあとで
...
;残り全部
    ...
    SUB ECX,512/4 ;IPL分を差し引く
    CALL memcpy

;asmheadでしなければいけないことおわり
;あとはbootpackに任せる
...
waitkbdout:
    IN AL,0x64
    AND AL,0x02
    JNZ waitkbdout ;ANDの結果!=0ならwaitkbdoutへ
    RET
...
GDT0:
    ...
    DW 0xffff,0x0000,0x9a28,0x0047 ;実行可能セグメント32bit(bootpaxkよう)
