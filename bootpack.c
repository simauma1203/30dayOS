...
void HariMain(void){
    ...
    struct BOOTINFO *binfo=(struct BOOTINFO *)ADR_BOOTINFO;//番地変数　0xff0から12BはBOOTINFO
    struct FIFO32 fifo;
    char s[40];
    int fifobuf[128];
    ...
    sheet_setbuf(sht_back,buf_back,binfo->scrnx,binfo->scrny,-1);//透明なし
    ...
    make_window8(buf_win,160,68,"(^_^)b");
